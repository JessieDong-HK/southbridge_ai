---
name: xhs
description: 提取小红书帖子内容（文字、图片OCR、视频转录），整理为 Markdown 并保存
user-invocable: true
argument-hint: <小红书链接>
...
# 保存 note 数据到 /tmp 供后续步骤（OCR/转录）使用
note = data['note']['noteDetailMap'][list(data['note']['noteDetailMap'].keys())[0]]['note']
with open('/tmp/xhs_note_{post_id}.json', 'w') as f:
    json.dump(note, f, ensure_ascii=False)
print(json.dumps({'status': 'ok', 'type': note.get('type'), 'title': note.get('title')}))
```
...
### 步骤 3B：图片文字识别（仅图文帖）
如果帖子 type 不是 video（即图文帖 "normal"），且 imageList 非空，执行以下**一体化脚本**（下载 → OCR → 清理，一步完成）：

```bash
PID="<post_id>"

# 从步骤 2 的 Python 输出中获取 imageList JSON，逐张下载
python3 -c "
import json, sys
with open('/tmp/xhs_note_${PID}.json') as f:
    note = json.load(f)
for i, img in enumerate(note.get('imageList', [])):
    url = img.get('urlDefault', '') if isinstance(img, dict) else img
    if url:
        print(f'{i}|{url}')
" | while IFS='|' read idx url; do
  curl -s -L -o \"/tmp/xhs_img_${PID}_\${idx}.jpg\" \
    -H \"Referer: https://www.xiaohongshu.com/\" \"\$url\" &
done
wait

# OCR 识别（macOS Vision 框架，离线，零 token）
~/bin/xhs-ocr /tmp/xhs_img_${PID}_*.jpg > /tmp/xhs_ocr_${PID}.txt 2>&1

# 清理图片
rm -f /tmp/xhs_img_${PID}_*.jpg

# 输出 OCR 结果
cat /tmp/xhs_ocr_${PID}.txt
```

注意：步骤 2 的 Python 脚本需要额外将 note 数据保存到 `/tmp/xhs_note_{post_id}.json`，供本步骤使用。

如 `~/bin/xhs-ocr` 文件缺失，跳过 OCR 并在笔记中注明"OCR 工具未安装，图片文字未提取"。
...
> [!quote]- 原文
> 帖子的完整原始内容，包括两个来源：
> - **desc 字段**：原帖配文，一字未改，保留 emoji 和换行，仅清理 `#xxx[话题]#` 标记
> - **图片 OCR**：如果帖子有图片且成功运行了 OCR，将识别出的文字附在此处（标注 `[图N 识别文字]`）
> - 不做任何总结、改写、删减
