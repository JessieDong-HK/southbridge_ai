...
function createStudentFolders(studentName, identity) {
  const dirs = [
    `${studentName}/会议逐字稿`,
    `${studentName}/评估与建议`,
    `${studentName}/微信聊天记录`,
  ...
  for (const d of dirs) {
    const full = path.join(CONSULTING_DIR, d);
    if (!fs.existsSync(full)) fs.mkdirSync(full, { recursive: true });
  }
  // Create identity-specific chat record file if it doesn't exist
  const chatFile = identity === 'student'
    ? `${studentName}/微信聊天记录/学生本人聊天记录.md`
    : `${studentName}/微信聊天记录/学生家长聊天记录.md`;
  const chatFull = path.join(CONSULTING_DIR, chatFile);
  if (!fs.existsSync(chatFull)) {
    const header = identity === 'student'
      ? `# ${studentName} · 学生本人微信聊天记录\n\n> 最后更新：${new Date().toISOString().slice(0, 10)}\n\n---\n\n`
      : `# ${studentName} · 学生家长微信聊天记录\n\n> 最后更新：${new Date().toISOString().slice(0, 10)}\n\n---\n\n`;
    fs.writeFileSync(chatFull, header, 'utf-8');
  }
}
...
// ── 添加客户（蓝图 v2：5 必填项）──
app.post('/api/add-client', upload.fields([
  { name: 'profileScreenshot', maxCount: 1 },
  { name: 'chatScreenshot', maxCount: 1 },
]), async (req, res) => {
  try {
    const { student, identity, screeningResult } = req.body;
    const profileFile = req.files?.profileScreenshot?.[0];
    const chatFile = req.files?.chatScreenshot?.[0];

    // Validate all 5 required fields
    if (!student || !profileFile || !chatFile || !identity || !screeningResult) {
      return res.status(400).json({ error: '缺少必填项。需要：学生姓名、微信身份截图、聊天记录截图、身份选择、筛查分类' });
    }
    if (!['parent','student'].includes(identity)) {
      return res.status(400).json({ error: '身份选择无效，需为 parent 或 student' });
    }
    if (!['excellent','qualified','unqualified'].includes(screeningResult)) {
      return res.status(400).json({ error: '筛查分类无效' });
    }

    const screeningLabel = { excellent: '优秀', qualified: '过bar', unqualified: '不满足' }[screeningResult];
    const identityLabel = identity === 'student' ? '学生本人' : '学生家长';
    const date = new Date().toISOString().slice(0, 10);
    const now = new Date().toISOString();

    // 1. Create folder structure
    createStudentFolders(student, identity);

    // 2. Save screenshots to 微信聊天记录/
    const screenshotDir = path.join(CONSULTING_DIR, student, '微信聊天记录');
    const profileDest = `${date}-微信身份页.png`;
    const chatDest = `${date}-聊天记录截图.png`;
    fs.copyFileSync(profileFile.path, path.join(screenshotDir, profileDest));
    fs.copyFileSync(chatFile.path, path.join(screenshotDir, chatDest));

    // 3. Create initial 学生档案.md from template
    const tmpl = fs.readFileSync(path.join(CONSULTING_DIR, '_模板', '学生档案.md'), 'utf-8');
    const profile = tmpl
      .replace(/name:.*/, `name: "${student}"`)
      .replace(/status:.*/, `status: "初步接触"`)
      .replace(/created:.*/, `created: "${date}"`)
      .replace(/updated:.*/, `updated: "${date}"`)
      .replace(/id:.*/, `id: "STUDENT-${String(Date.now()).slice(-6)}"`)
      .replace(/# .+/, `# ${student}`)
      .replace(/> .+/, `> ${identityLabel} · ${screeningLabel} · ${date} 建档`);
    writeFileLocal(`${student}/学生档案.md`, profile);

    // 4. Append initial chat record entry
    const chatRecordFile = identity === 'student'
      ? `${student}/微信聊天记录/学生本人聊天记录.md`
      : `${student}/微信聊天记录/学生家长聊天记录.md`;
    let chatContent = fs.readFileSync(path.join(CONSULTING_DIR, chatRecordFile), 'utf-8');
    chatContent += `\n## ${date} · 首次建档\n\n`;
    chatContent += `- **身份**：${identityLabel}\n`;
    chatContent += `- **筛查分类**：${screeningLabel}\n`;
    chatContent += `- **微信身份截图**：${profileDest}\n`;
    chatContent += `- **聊天记录截图**：${chatDest}\n`;
    chatContent += `- **建档时间**：${now}\n`;
    writeFileLocal(chatRecordFile, chatContent);

    // 5. Cleanup temp files
    [profileFile, chatFile].forEach(f => {
    ...
    res.json({
      ...
      hint: '客户已建档。后续上传聊天截图将自动追加到此客户记录。',
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/upload-chat', upload.single('screenshot'), async (req, res) => {
  try {
    ...
    // Save screenshot to 待处理截图 for later AI processing
    ...
    // Write trigger file for chat processing workflow
    const triggerContent = [
      `student: ${student}`,
      `identity: ${identity || 'unknown'}`,
      `screenshotPath: ${student}/微信聊天记录/待处理截图/${destName}`,
      `triggeredAt: ${new Date().toISOString()}`,
    ].join('\n');
    await writeFile(`${student}/_trigger_chat_processing`, triggerContent);

    res.json({ ok:true, student, savedTo:`${student}/微信聊天记录/待处理截图/${destName}`, hint: '已触发聊天记录处理' });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/upload-transcript', upload.single('transcript'), async (req, res) => {
  try {
    const { student, meetingNumber, meetingType, attendees } = req.body;
    ...
    // Write trigger file for transcript processing workflow
    const triggerContent = [
      `student: ${student}`,
    ...
    await writeFile(`${student}/_trigger_transcript_processing`, triggerContent);

    // Clean up temp upload
    if (!IS_VERCEL && fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);

    res.json({ ok: true, student, transcriptPath: destPath, hint: '已触发会议记录处理，约1-2分钟后可在评估建议Tab查看' });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
