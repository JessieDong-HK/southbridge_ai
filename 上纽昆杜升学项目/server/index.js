...
const AI_RETRY_COUNT = Number(process.env.AI_RETRY_COUNT || 3);
const AI_RETRY_BASE_DELAY_MS = Number(process.env.AI_RETRY_BASE_DELAY_MS || 3000);
const AI_RETRY_MAX_DELAY_MS = Number(process.env.AI_RETRY_MAX_DELAY_MS || 60000);
const ASSESSMENT_TIMEOUT_MS = Number(process.env.ASSESSMENT_TIMEOUT_MS || Math.min(AI_TIMEOUT_MS, 90000));
const ASSESSMENT_RETRY_COUNT = Number(process.env.ASSESSMENT_RETRY_COUNT || 1);
const ASSESSMENT_MAX_TOKENS = Number(process.env.ASSESSMENT_MAX_TOKENS || 1600);
const TRANSCRIPT_AI_CHARS = Number(process.env.TRANSCRIPT_AI_CHARS || 3000);
const ASSESSMENT_TRANSCRIPT_CHARS = Number(process.env.ASSESSMENT_TRANSCRIPT_CHARS || 800);
const ANALYSIS_SUMMARY_CHARS = Number(process.env.ANALYSIS_SUMMARY_CHARS || 2400);
const ANALYSIS_PROGRESS_CHARS = Number(process.env.ANALYSIS_PROGRESS_CHARS || 1600);
const ANALYSIS_GLOBAL_CHARS = Number(process.env.ANALYSIS_GLOBAL_CHARS || 1200);
const CHAT_ANALYSIS_CHARS = Number(process.env.CHAT_ANALYSIS_CHARS || 8000);
const POST_PROCESS_TIMEOUT_MS = Number(process.env.POST_PROCESS_TIMEOUT_MS || Math.min(AI_TIMEOUT_MS, 60000));
const POST_PROCESS_RETRY_COUNT = Number(process.env.POST_PROCESS_RETRY_COUNT || 0);
const OCR_IMAGE_MAX_WIDTH = Number(process.env.OCR_IMAGE_MAX_WIDTH || 900);
const OCR_IMAGE_JPEG_QUALITY = Number(process.env.OCR_IMAGE_JPEG_QUALITY || 78);
const VISION_RETRY_COUNT = Number(process.env.VISION_RETRY_COUNT || Math.max(AI_RETRY_COUNT, 8));
const VISION_RETRY_BASE_DELAY_MS = Number(process.env.VISION_RETRY_BASE_DELAY_MS || 15000);
const VISION_RETRY_MAX_DELAY_MS = Number(process.env.VISION_RETRY_MAX_DELAY_MS || 120000);
...
app.post('/api/upload-chat', upload.array('screenshots', 4), async (req, res) => {
  try {
    ...
    for (const [index, file] of req.files.entries()) {
      ...
      saved.push(await prepareOcrImage(dest));
    }
...
async function processChatUpload(taskId, student, identity, imagePaths) {
  ...
  for (const [index, imagePath] of imagePaths.entries()) {
    updateTask(taskId, 'running', `Step 1/2: OCR 逐字提取截图文字（${index + 1}/${imagePaths.length}）`);
    const text = await callVision(ocrPrompt, [imagePath]);
    ocrParts.push(`--- 截图 ${index + 1}/${imagePaths.length} ---\n${text}`);
  }
  const ocrRaw = ocrParts.join('\n\n');
  ...
  const step2RawParts = [];
  let messages = [];
  for (const [index, part] of ocrParts.entries()) {
    updateTask(taskId, 'running', `Step 2/2: 整理截图 OCR 为结构化聊天记录（${index + 1}/${ocrParts.length}）`);
    const step2Raw = await structureOcrChunk(part);
    step2RawParts.push(`--- 截图 ${index + 1}/${ocrParts.length} ---\n${step2Raw}`);
    if (step2Raw && step2Raw.length > 10 && !step2Raw.startsWith('API 调用失败')) {
      try {
        const structured = parseJsonFromText(step2Raw);
        if (Array.isArray(structured.messages)) messages.push(...structured.messages);
      } catch (e) {
        updateTask(taskId, 'running', `截图 ${index + 1} JSON 解析失败（${e.message.slice(0,50)}），继续处理其他截图`);
      }
    }
  }
  const step2Raw = step2RawParts.join('\n\n');
  ...
  try {
    const latestMessages = compactLongText(messagesToMarkdown(messages), CHAT_ANALYSIS_CHARS);
    const analysis = await analyzeStudentMaterial(
      student,
      `本次新增微信聊天记录：\n${latestMessages}`,
      'chat',
      { timeoutMs: POST_PROCESS_TIMEOUT_MS, retryCount: POST_PROCESS_RETRY_COUNT }
    );
    applyStudentAnalysis(student, analysis);
  } catch (error) {
    updateTask(taskId, 'running', `聊天记录已保存，档案自动更新失败：${shortError(error)}`);
  }

  completeTask(taskId, `聊天处理完成，新增 ${added} 条消息（OCR 原始输出已保存）`);
}

async function structureOcrChunk(ocrText) {
  const step2Prompt = [
  ...
  try {
    return await callClaudeText(step2Prompt);
  } catch (e) {
...
async function processTranscriptUpload(taskId, student, num, attendees, tmpPath) {
  ...
  const cleanedForAi = compactLongText(cleaned, TRANSCRIPT_AI_CHARS);
  const prompt = [
    `学生：${student}`,
    `场次：第${meetingNum}场`,
    `参会人：${attendees.join(', ')}`,
    '以下是清洗后的会议逐字稿节选；完整原文已保存到学生文件夹，请基于节选提炼档案更新和客服下一步动作：',
    cleanedForAi,
  ].join('\n');
  try {
    const analysis = await analyzeStudentMaterial(
    ...
    const fallbackSummary = buildTranscriptFallbackSummary(student, meetingNum, attendees, cleaned, error);
    fs.writeFileSync(
      path.join(STUDENTS_DIR, student, '会议逐字稿', `第${meetingNum}场-会议总结-客服查看.md`),
      fallbackSummary,
      'utf-8'
    );
    completeTask(taskId, `第 ${meetingNum} 场会议逐字稿已保存，AI 总结/档案更新失败：${shortError(error)}`);
  }
}
...
async function processAssessment(taskId, student) {
  ...
  if (fs.existsSync(transcriptDir)) {
    for (const file of fs.readdirSync(transcriptDir).filter((f) => f.endsWith('.md') && !f.includes('会议总结') && !f.includes('客服查看')).sort()) {
      const content = fs.readFileSync(path.join(transcriptDir, file), 'utf-8');
      const excerpt = compactLongText(content, ASSESSMENT_TRANSCRIPT_CHARS);
      transcriptEntries.push({ file, excerpt });
      material += `\n\n===== ${file} =====\n${excerpt}`;
    }
  }
  ...
  const summaryBrief = compactLongText(stripFrontmatter(summary).replace(/\n{3,}/g, '\n\n'), ANALYSIS_SUMMARY_CHARS);
  const prompt = [
    '你是南桥教育的资深升学顾问。请生成一份仅供客服查看的评估与建议速览，不要写成直接发送给家长的口吻。',
    `学生：${student}`,
    '学生信息摘要：',
    summaryBrief || '暂无',
    '会议资料短节选：',
    material,
    '输出 Markdown，控制在 1200 字以内，结构必须包含：学生画像速览、匹配度分析（上纽/昆杜）、申请时间线、需要突破的方向、推荐方案、下一步客服动作。',
    '如果信息不足，不要编造，用“待确认”标注。',
  ].join('\n');
  let content;
  let completionMessage = '评估与建议已生成';
  try {
    content = await callClaudeText(prompt, [], {
      timeoutMs: ASSESSMENT_TIMEOUT_MS,
      retryCount: ASSESSMENT_RETRY_COUNT,
      maxTokens: ASSESSMENT_MAX_TOKENS,
    });
  } catch (error) {
    content = buildAssessmentFallback(student, summary, transcriptEntries, error);
    completionMessage = `评估与建议已生成（AI 暂时失败，已生成本地兜底草稿：${shortError(error)}）`;
  }
  ...
  completeTask(taskId, completionMessage);
}

function buildTranscriptFallbackSummary(student, meetingNum, attendees, cleaned, error) {
  const excerpt = compactLongText(cleaned, 1800);
  return [
    `# ${student} · 第${meetingNum}场会议总结（客服查看）`,
    '',
    `> 生成时间：${currentDateTime()}`,
    `> 说明：AI 会议总结/档案更新暂时失败，系统已保存清洗后的完整逐字稿，并生成本地兜底摘要供客服先复核。失败原因：${shortError(error)}`,
    '',
    '## 会议信息',
    `- 场次：第${meetingNum}场`,
    `- 参会人：${attendees.join(', ') || '待确认'}`,
    '- 完整资料：请打开同目录下对应 `YYYY-MM-DD-第X场.md` 逐字稿复核',
    '',
    '## 客服先看',
    '- 本摘要由系统兜底生成，不替代顾问判断。',
    '- 请客服先从逐字稿中确认学生基础信息、目标校、家庭顾虑、下次会议/跟进时间。',
    '- 如果评估或档案更新失败，可稍后在 AI 服务稳定后重新上传或重新触发评估。',
    '',
    '## 待确认',
    '- 学生年级、学校、课程体系、校内成绩/排名',
    '- 英语/标化成绩、竞赛/活动/夏校经历',
    '- 目标校优先级、家庭决策人、预算与签约阻力',
    '- 下一次会议时间、参会老师、会议链接和会后交付物',
    '',
    '## 逐字稿节选',
    excerpt || '暂无可用逐字稿内容。',
  ].join('\n');
...
function buildAssessmentFallback(student, summary, transcriptEntries, error) {
  const summaryPreview = compactLongText(stripFrontmatter(summary).replace(/\n{3,}/g, '\n\n'), 1200)
    || '暂无可用的结构化学生总结。请客服优先复核微信聊天记录和会议逐字稿。';
  const meetingList = transcriptEntries.length
    ? transcriptEntries.map((entry, index) => `- 第 ${index + 1} 份资料：${entry.file}（完整原文已保存，可在学生文件中打开复核）`).join('\n')
    : '- 暂无会议逐字稿。';
  const excerpts = transcriptEntries.length
    ? transcriptEntries.map((entry) => `### ${entry.file}\n${entry.excerpt}`).join('\n\n')
    : '暂无逐字稿节选。';
  ...
  return [
    ...
    '',
    `> 生成时间：${currentDateTime()}`,
    `> 说明：AI 评估接口暂时失败，系统已生成本地兜底草稿，供客服先推进流程。失败原因：${shortError(error)}`,
    '',
    '## 学生画像速览',
    summaryPreview,
    '',
    '## 匹配度分析（上纽/昆杜）',
    '- 已有聊天记录和会议逐字稿可以作为后续评估依据。',
    '- 当前兜底草稿不替代顾问判断；请客服结合学生成绩、英语能力、活动经历、文书素材和家庭预期复核上纽/昆杜匹配度。',
    '- 若逐字稿中已出现明确目标校、预算、时间节点或顾虑，请优先整理为下一次跟进重点。',
    '',
    '## 申请时间线',
    meetingList,
    '- 下一步建议：先完成资料核对，再安排顾问输出正式评估结论和行动计划。',
    '',
    '## 需要突破的方向',
    '- 补齐学生基础信息：年级、学校、课程体系、校内成绩、英语/标化、活动与竞赛。',
    '- 从逐字稿中提炼核心动机、家庭决策人、主要顾虑、预算和签约阻力。',
    '- 明确最紧急的申请节点，并把下一步动作拆成可执行待办。',
    '',
    '## 推荐方案',
    '- 客服先复核原始逐字稿，确认是否需要补问关键资料。',
    '- 若资料已足够，安排顾问给出正式评估并进入「开会与发评估」后的跟进。',
    '- 若资料不足，先发起一次短会或微信补问，再重新触发评估生成。',
    '',
    '## 下一步客服动作',
    '- 打开学生文件夹，检查会议逐字稿原文和微信聊天记录是否完整。',
    '- 将本草稿中的缺口整理为待办，避免因为 AI 接口失败导致流程停住。',
    '- 稍后 AI 服务稳定后，可重新点击生成评估，覆盖为正式 AI 版本。',
    '',
    '## 原始逐字稿节选（便于复核）',
    excerpts,
  ].join('\n');
...
async function analyzeStudentMaterial(student, material, source, options = {}) {
  const summary = readStudentPreferred(student, '学生信息总结.md', '学生档案.md') || '';
  ...
  const summaryBrief = compactLongText(stripFrontmatter(summary).replace(/\n{3,}/g, '\n\n'), ANALYSIS_SUMMARY_CHARS);
  const progressBrief = compactLongText(stripFrontmatter(progress).replace(/\n{3,}/g, '\n\n'), ANALYSIS_PROGRESS_CHARS);
  const studentTodosBrief = compactLongText(extractStudentRelatedSection(todos, student), ANALYSIS_GLOBAL_CHARS);
  const studentMeetingsBrief = compactLongText(extractStudentRelatedRows(meetings, student), ANALYSIS_GLOBAL_CHARS);
  const prompt = [
    '你是南桥教育客服工作台的资料更新助手。所有输出只供客服内部查看，不直接触达家长。',
    `学生：${student}`,
    `资料来源：${source}`,
    '现有学生信息总结/旧档案（已截断，保留关键事实）：',
    summaryBrief || '暂无',
    '现有客户咨询进度（已截断）：',
    progressBrief || '暂无',
    '该学生相关待办（不要覆盖其他学生）：',
    studentTodosBrief || '暂无',
    '该学生相关会议安排（不要覆盖其他学生）：',
    studentMeetingsBrief || '暂无',
    '本次新资料：',
    material,
    '请输出 JSON，不要包裹 Markdown 代码块。字段：',
    '{',
    ' "studentSummary": "更新后的学生信息总结 Markdown，保留关键事实，控制在 1200 字以内",',
    ' "progress": "更新后的客户咨询进度 Markdown，frontmatter 含 stage 和 updated，控制在 800 字以内",',
    ' "todos": "只输出该学生相关待办 Markdown，标题为 # 待办清单，最多 8 条；不要写其他学生",',
    ' "meetings": "只输出该学生相关会议 Markdown 表格，标题为 # 会议安排，最多 5 行；不要写其他学生",',
    ' "internalMeetingSummary": "如资料是会议，生成内部会议总结 Markdown，最多 900 字；否则为空",',
    ' "customerVisibleMeetingSummary": "如资料是会议，生成客服可查看的简明总结 Markdown，最多 500 字；否则为空"',
    '}',
    '如果信息不足，不要编造，用“待确认”标注。',
    '阶段只能是：添加微信、筛查、开会与发评估、跟进、已签约、已流失。未回复不能判为已流失。',
  ].join('\n');
  return callClaudeJson(prompt, [], options);
}
...
function applyStudentAnalysis(student, analysis) {
  ...
  if (analysis.todos) mergeStudentTodos(student, analysis.todos);
  if (analysis.meetings) mergeStudentMeetings(student, analysis.meetings);
}

function extractStudentRelatedSection(content, student) {
  ...
  for (const line of lines) {
    ...
    if (heading) {
      inStudentSection = heading[1].trim() === student;
      if (inStudentSection) result.push('', line);
      continue;
    }
    const isTodo = /^-\s+\[[ xX]\]\s+/.test(line.trim()) || /^-\s+/.test(line.trim());
    if (inStudentSection || (isTodo && line.includes(student))) result.push(line);
  }
  const text = result.join('\n').trim();
  if (text !== '# 待办清单') return text;
  const parsed = parseTodoMarkdown(content).filter((todo) => todo.student === student);
  if (parsed.length) {
    return ['# 待办清单', '', `## ${student}`, ...parsed.map((todo) => `- [${todo.done ? 'x' : ' '}] ${todo.text}`)].join('\n');
  }
  return compactLongText(content, ANALYSIS_GLOBAL_CHARS);
}

function extractStudentRelatedRows(content, student) {
  const rows = [];
  for (const line of String(content || '').split(/\r?\n/)) {
    if (!line.trim().startsWith('|') || line.includes('---') || line.includes('时间')) continue;
    const cols = line.split('|').slice(1, -1).map((c) => c.trim());
    if (cols.length >= 4 && (cols[1] === student || line.includes(student))) rows.push(`| ${cols[0]} | ${cols[1]} | ${cols[2]} | ${cols[3]} |`);
  }
  return ['# 会议安排', '', '| 时间 | 学生 | 参会人 | 状态 |', '|------|------|--------|------|', ...rows].join('\n');
}

function mergeStudentTodos(student, incoming) {
  const items = extractTodoItems(incoming).slice(0, 8);
  if (!items.length) return;

  const current = readGlobalFile('待办清单.md') || '# 待办清单\n';
  const output = [];
  let skippingStudentSection = false;
  for (const line of current.split(/\r?\n/)) {
    const heading = line.match(/^##\s+(.+)/);
    if (heading) {
      skippingStudentSection = heading[1].trim() === student;
      if (skippingStudentSection) continue;
    }
    if (skippingStudentSection) continue;
    const explicitStudentTodo = /^-\s+\[[ xX]\]\s+/.test(line.trim()) && line.includes(student);
    if (!explicitStudentTodo) output.push(line);
  }

  let base = output.join('\n').trim();
  if (!base.startsWith('# 待办清单')) base = `# 待办清单\n\n${base}`.trim();
  const section = [
    `## ${student}`,
    ...items.map((item) => `- [${item.done ? 'x' : ' '}] ${stripStudentPrefix(item.text, student)}`),
  ].join('\n');
  writeGlobalFile('待办清单.md', `${base}\n\n${section}`);
}
...
function extractTodoItems(markdown) {
  ...
  for (const line of String(markdown || '').split(/\r?\n/)) {
    ...
    const text = String((checkbox ? checkbox[2] : bullet[1]) || '').trim();
...
function stripStudentPrefix(text, student) {
  return String(text || '').replace(new RegExp(`^${escapeRegExp(student)}\\s*[：:、-]?\\s*`), '').trim();
}

function mergeStudentMeetings(student, incoming) {
  const incomingRows = extractMeetingRows(incoming, student).slice(0, 5);
  if (!incomingRows.length) return;

  const current = readGlobalFile('会议安排.md') || '';
  const rows = [];
  for (const line of current.split(/\r?\n/)) {
    if (!line.trim().startsWith('|') || line.includes('---') || line.includes('时间')) continue;
    const cols = line.split('|').slice(1, -1).map((c) => c.trim());
    if (cols.length >= 4 && cols[1] !== student) rows.push(cols);
  }
  ...
  writeGlobalFile('会议安排.md', lines.join('\n'));
}

function extractMeetingRows(markdown, student) {
  const rows = [];
  const seen = new Set();
  for (const line of String(markdown || '').split(/\r?\n/)) {
    if (!line.trim().startsWith('|') || line.includes('---') || line.includes('时间')) continue;
    ...
    const row = [cols[0] || '待确认', cols[1] || student, cols[2] || '待确认', cols[3] || '待确认'];
    row[1] = student;
    const key = row.join('|');
...
async function callClaudeText(prompt, imageParts = [], options = {}) {
  if (!AI_KEY) throw new Error('未配置 ANTHROPIC_API_KEY');
  if (AI_STYLE === 'anthropic') return callAnthropicMessages(prompt, imageParts, options);
  return callOpenAiCompatible(prompt, imageParts, options);
}

async function callClaudeJson(prompt, imageParts = [], options = {}) {
  const text = await callClaudeText(`${prompt}\n\n只输出可解析 JSON。`, imageParts, options);
  return parseJsonFromText(text);
}

async function callAnthropicMessages(prompt, imageParts = [], options = {}) {
  const timeoutMs = options.timeoutMs || AI_TIMEOUT_MS;
  const retryCount = Number.isFinite(options.retryCount) ? options.retryCount : AI_RETRY_COUNT;
  const maxTokens = Number(options.maxTokens || process.env.ANTHROPIC_MAX_TOKENS || 4096);
  const payload = await fetchJsonWithRetry(`${AI_BASE_URL}/v1/messages`, {
    method: 'POST',
    headers: {
      ...getAuthHeaders(),
...
async function callOpenAiCompatible(prompt, imageParts = [], options = {}) {
  const timeoutMs = options.timeoutMs || AI_TIMEOUT_MS;
  const retryCount = Number.isFinite(options.retryCount) ? options.retryCount : AI_RETRY_COUNT;
  const maxTokens = Number(options.maxTokens || process.env.ANTHROPIC_MAX_TOKENS || 0);
  ...
  const payload = await fetchJsonWithRetry(`${AI_BASE_URL}/v1/chat/completions`, {
    method: 'POST',
    headers: {
      ...getAuthHeaders(),
      'content-type': 'application/json',
...
async function callVision(prompt, imagePaths) {
  ...
  const payload = await fetchJsonWithRetry(`${VISION_BASE_URL}/chat/completions`, {
    method: 'POST',
  ...
  }, Number(process.env.VISION_TIMEOUT_MS || AI_TIMEOUT_MS), VISION_RETRY_COUNT, {
    baseDelayMs: VISION_RETRY_BASE_DELAY_MS,
    maxDelayMs: VISION_RETRY_MAX_DELAY_MS,
  });
...
async function fetchJsonWithRetry(url, options, timeoutMs, retryCount = AI_RETRY_COUNT, retryOptions = {}) {
  let lastError;
  const baseDelayMs = retryOptions.baseDelayMs || AI_RETRY_BASE_DELAY_MS;
  const maxDelayMs = retryOptions.maxDelayMs || AI_RETRY_MAX_DELAY_MS;
  for (let attempt = 0; attempt <= retryCount; attempt += 1) {
    try {
      ...
      if (!isRetryableAiError(error) || attempt === retryCount) throw error;
      lastError = error;
    } catch (error) {
      if (!isRetryableAiError(error) || attempt === retryCount) throw error;
      lastError = error;
...
async function readJsonResponse(response) {
  ...
  try {
    ...
    return { error: { message: text.slice(0, 1000) }, raw: text };
  }
}
...
function messagesToMarkdown(messages) {
  return messages
    ...
    .map((m) => `**${m.datetime || currentDateTime()}** ${m.speaker || '未知'}：\n${m.content}`)
    .join('\n\n');
}
...
function userFacingAiError(error) {
  ...
  if (text.includes('额度不足') || text.includes('余额不足') || lower.includes('insufficient') || lower.includes('quota')) {
    return `AI 接口额度不足，请更换或充值 OCR/文本模型 Key 后重试。本次任务未完成：${text}`;
  }
  if (lower.includes('concurrency limit') || lower.includes('too many pending')) {
    return `AI 接口并发限制，请稍后重试或降低同时上传量。本次任务未完成：${text}`;
  }
  if (lower.includes('gateway time-out') || lower.includes('timeout') || lower.includes('timed out') || text.includes('超时')) {
    return `AI 接口超时，请稍后重试；如果聊天记录或逐字稿已保存，可先继续查看已落盘文件。本次错误：${text}`;
  }
  return text || 'AI 任务失败，请查看服务器日志';
}

function compactLongText(text, maxChars) {
  const value = String(text || '').trim();
  if (value.length <= maxChars) return value;
  const half = Math.floor((maxChars - 120) / 2);
  return [
    value.slice(0, half),
    `\n\n...（中间 ${value.length - half * 2} 字已省略，原文已保存，可在文件中查看）...\n\n`,
    value.slice(-half),
  ].join('');
}
...
async function prepareOcrImage(originalPath) {
  const ext = path.extname(originalPath).toLowerCase();
  const ocrPath = originalPath.replace(new RegExp(`${escapeRegExp(ext)}$`), `.ocr.jpg`);
  try {
    const metadata = await sharp(originalPath).metadata();
    const needsResize = Number(metadata.width || 0) > OCR_IMAGE_MAX_WIDTH;
    let pipeline = sharp(originalPath, { limitInputPixels: false }).rotate();
    if (needsResize) {
      pipeline = pipeline.resize({ width: OCR_IMAGE_MAX_WIDTH, withoutEnlargement: true });
    }
    await pipeline
      .jpeg({ quality: OCR_IMAGE_JPEG_QUALITY, mozjpeg: true })
      .toFile(ocrPath);
    return ocrPath;
  } catch (error) {
    console.warn(`OCR image optimize failed for ${originalPath}: ${error.message}`);
    return originalPath;
  }
}
...
function failTask(id, error) {
  updateTask(id, 'failed', userFacingAiError(error));
}
